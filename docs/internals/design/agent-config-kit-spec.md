...
| **Pi** | global `~/.pi/agent/mcp.json` → `mcpServers`; project `.pi/mcp.json` (both scopes populated; originally specified as `.pi/settings.json`, corrected after verifying pi-mcp-adapter's `getProjectPiConfigPath()` reads `.pi/mcp.json`) | `mcp_conditional_on="requires the third-party pi-mcp-adapter Pi package; Pi core has no MCP support, so writing this file is a silent no-op without it"` | `PluginRegistration` (`.ts` files) into `~/.pi/agent/extensions/` | `~/.pi/agent/skills/` only — Pi natively unions this with `~/.agents/skills/` itself, so writing there too would duplicate the skill and trigger Pi's own name-collision warning |
