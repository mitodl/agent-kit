...
| 5 | Required configuration read with `.get()` / `config.get()` so a missing value becomes `None` | Correctness | The input that's missing and what then deploys or runs with `None`. Use `mapping["key"]`, `os.environ["X"]`, or `config.require()` (`config.require_secret()` for a secret that feeds a resource input or export directly; a `SecretStr` field on a component config model takes `require()`) |
