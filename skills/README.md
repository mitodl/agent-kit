...
| process | [`screenshot-pr`](./process/screenshot-pr/SKILL.md) | Capture UI changes for a PR with a re-runnable Playwright script, each shot asserting on page content so a 5xx, an expired login or a stale build fails the run |
