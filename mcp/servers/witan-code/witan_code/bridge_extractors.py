...
# Patterns that identify generated or same-origin-only client code that should
# not produce cross-repo endpoint consumer bindings.

# k6 load-test generated clients import from "k6/*" — auto-generated by
# @grafana/openapi-to-k6 from the repo's own spec; not real cross-repo calls.
_K6_IMPORT = re.compile(r"""(?:from|require\()\s*[\"'](k6/)""")

# redux-hammock's fetchJSONWithCSRF makes same-origin Django CSRF requests.
# The URL it receives is a relative path to the same app, never a foreign service.
_CSRF_IMPORT = re.compile(
    r"""(?:from|require\()\s*[\"'](redux-hammock/django_csrf_fetch)[\"']"""
)


def _is_generated_client(text: str, file: str) -> bool:
    """Return True if this file is generated/load-test code that should be skipped.

    Matches:
    - Files under a ``load_testing/`` directory tree.
    - TypeScript/JavaScript files that import from ``k6/*`` (openapi-to-k6 output).
    """
    if "load_testing/" in file or file.startswith("load_testing/"):
        return True
    if _K6_IMPORT.search(text):
        return True
    return False


def _is_csrf_only_client(text: str) -> bool:
    """Return True if this file exclusively makes same-origin CSRF fetches.

    Files importing ``redux-hammock/django_csrf_fetch`` call their own Django
    backend via relative paths — these are never cross-repo endpoint consumers.
    """
    return bool(_CSRF_IMPORT.search(text))
...
def extract_file_bindings(text: str, language: str, file: str) -> list[ParsedBinding]:
    ...
    Files identified as generated load-test clients (``load_testing/`` paths or
    ``k6/*`` imports) are skipped entirely — they mirror the repo's own routes
    and would produce false cross-repo endpoint edges.

    Files that import ``redux-hammock/django_csrf_fetch`` are skipped for
    endpoint consumers — their URL arguments are same-origin relative paths that
    happen to collide with foreign providers' normalized keys.
    """
    ...
    if language == "python":
        ...
        # Skip generated / load-test client files entirely.
        if _is_generated_client(text, file):
            return out
        skip_endpoints = _is_csrf_only_client(text)
        for pat in _TS_ENV:
            for m in pat.finditer(text):
