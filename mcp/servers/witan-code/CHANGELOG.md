...
- **Raised the `witan-core` floor to `>=0.32` — a RUNTIME floor.** `setup` now
  ...
  setup runs** — the module imports fine and every test passes, which is
  exactly the case `just check-core-floor` documents itself as unable to catch.
  ...
  0.31.0 while this was open. This floor has now moved twice on rebase and the
  pyproject line auto-merged silently both times, since both sides named the
  same number — re-check it by hand on every rebase.
