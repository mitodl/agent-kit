...
places the binary at witan/_bin/omnigraph inside the package. OmnigraphClient
checks there first before falling back to a PATH lookup, so the MCP server
works out of the box after `uvx --from git+... witan serve` with no separate
install step.
...
class CustomBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:
        ...
        pkg_bin = Path(self.root) / "witan" / "_bin"
