...
- **`witan tasks --ready --status X` filters by status.** `task_ready` takes
  no status, so `--status` was ignored and `--ready --status open` also listed
  `blocked` tasks whose blockers had closed and `in_progress` tasks with lapsed
  leases.
...
<!-- scriv-end-here: a real `## [Unreleased]` heading below was never
renamed when it shipped, and scriv's version parser rejects any non-version
`## [...]` heading in the entries it scans. This walls off the rest of the
archive (everything below here is untouched history) rather than rewriting
