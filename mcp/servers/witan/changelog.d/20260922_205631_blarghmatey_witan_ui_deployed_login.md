...
- **The web UI logs in on a deployed witan.** When `/ui/config.json` names an
  ...
  without a prompt. A 401 restarts the login once, and a second 401 right
  after a fresh login stops with a message pointing at the Keycloak client's
  audience mapper instead of redirecting forever. `witan ui` against a local
