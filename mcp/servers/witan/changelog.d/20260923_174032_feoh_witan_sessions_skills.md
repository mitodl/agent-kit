...
- **Pi sessions now auto-close their WorkflowSession on shutdown.** The Pi
  ...
  `CLAUDE_SESSION_ID` from that child only), and the `witan-workflow`,
  `witan-project-tracker` and `witan-task` skills tell Pi agents to pass
  `$PI_SESSION_ID` as the `session_id`. Claude Code behaviour is unchanged:
