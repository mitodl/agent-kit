...
#
# Unlike omnigraph-memory, code stores are per-repo and created LAZILY on first
...
#   ./install.sh
#
# Override the code store directory (default ~/.local/share/omnigraph-memory/code):
#   export OMNIGRAPH_CODEGRAPH_DIR=/path/to/code-stores
set -euo pipefail

CODE_DIR="${OMNIGRAPH_CODEGRAPH_DIR:-${HOME}/.local/share/omnigraph-memory/code}"
...
echo "     uvx --from . omnigraph-codegraph-index index"
echo "     # or, once installed on PATH:  omnigraph-codegraph-index index"
echo ""
