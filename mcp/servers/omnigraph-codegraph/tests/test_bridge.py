...
def test_cross_repo_env_and_endpoint_linkage(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNIGRAPH_CODEGRAPH_REPO", "https://github.com/test/repo-a")
    monkeypatch.setattr(srv, "cfg", cfg_mod.load())
    srv._clients.clear()
    stats_a = _index(srv, indexer, a)
    assert stats_a["bindings"] > 0

    monkeypatch.setenv("OMNIGRAPH_CODEGRAPH_REPO", "https://github.com/test/repo-b")
    monkeypatch.setattr(srv, "cfg", cfg_mod.load())
    srv._clients.clear()
...
def test_bridge_tools_empty_without_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OMNIGRAPH_CODEGRAPH_REPO", "https://github.com/test/none")
    from omnigraph_codegraph import config as cfg_mod
    from omnigraph_codegraph import server as srv
