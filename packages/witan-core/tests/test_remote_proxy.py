...
def test_input_schema_read_from_tool():
    assert _tool_input_schema(_Tool()) == SCHEMA


def test_param_name_extraction():
    # The expression _invoke actually builds _param_names with.
    names = list(_tool_input_schema(_Tool()).get("properties", {}).keys())
    assert names == ["slug"]
