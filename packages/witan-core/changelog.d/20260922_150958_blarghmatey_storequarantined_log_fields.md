...
- **`Refusal.log_safe_attributes`: a refusal can log an identifier without
  ...
  prose. Names the line already uses (`tool`, `error_type`, ...) are refused, and
  the audit tables cover the declarations as they cover the opt-ins.

### Changed

- **`sidecar_operation_id` parses only an identifier.** The quoted id must be
  `[0-9A-Za-z_-]{1,64}` or it is not parsed, because it now reaches the log and
  the text around it can be a non-JSON HTTP body passed through verbatim. An
  unparsed id still leaves a `StoreQuarantined`, only without a named sidecar,
  as before.
