...
- **Optimistic-concurrency retries back off with full jitter.** The fixed
  ...
  random time up to `0.1 * 2**(attempt-1)`, capped at 1s, and stop rather
  than start a retry past the call's deadline when one is set.

- **A write that exhausts its conflict retries raises `WriteContention`.** It
  is a `RuntimeError` and a `Refusal`, so existing `except RuntimeError`
  callers are unaffected, fastmcp logs it at WARNING instead of reporting a
  Sentry ERROR, and the caller is told nothing was written and the write can
  be retried. A read that exhausts them still raises a plain `RuntimeError`.
