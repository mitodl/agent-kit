...
`RegistrationBundle`/`apply` machinery declaratively via a TOML manifest — the
`agent-kit` console script lives in the separate
[`agent-kit`](../agent-kit/README.md) package (this library is one of its
dependencies, alongside `cyclopts`/`rich` and the `witan`/`witan-code` MCP
servers):
...
See [`packages/agent-kit/README.md`](../agent-kit/README.md) for the manifest
