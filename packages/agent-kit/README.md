...
The `agent-kit` CLI — drives [`agent-config-kit`](../agent-config-kit/README.md)'s
`RegistrationBundle`/`apply` machinery declaratively via a TOML manifest —
bundled with the [`witan`](../../mcp/servers/witan/README.md) (PyPI:
`witan-council`) and [`witan-code`](../../mcp/servers/witan-code/README.md)
MCP servers, so one install gets you all three:
...
This package is the CLI surface only — the underlying models
(`StdioServer`/`RemoteServer`, `DeclarativeHook`/`PluginRegistration`,
`SkillSource`, ...) and the `apply`/`apply_all` orchestration it calls into
live in `agent-config-kit`, usable standalone from Python without this
package's `cyclopts`/`rich`/`witan`/`witan-code` dependencies — see
[`packages/agent-config-kit/README.md`](../agent-config-kit/README.md).
...
floors (`agent-config-kit>=0.4.0`, `witan-council>=0.2.0`, `witan-code>=0.2.0`
— no upper bound), so a new release of any of the three is picked up by a
fresh install automatically without needing a matching `agent-kit` release.
